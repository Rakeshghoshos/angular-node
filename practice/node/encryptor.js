...
const decrypt = (encryptedPassword) => {
  let decLayer1 = crypto_js.TripleDES.decrypt(encryptedPassword, process.env.PASSWORD_SECRET);
  let deciphertext1 = decLayer1.toString(crypto_js.enc.Utf8);

  let decLayer2 = crypto_js.DES.decrypt(deciphertext1, process.env.PASSWORD_SECRET);
  let deciphertext2 = decLayer2.toString(crypto_js.enc.Utf8);

  let decLayer3 = crypto_js.AES.decrypt(deciphertext2, process.env.PASSWORD_SECRET);
